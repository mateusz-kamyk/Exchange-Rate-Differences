...
def calculate_difference(invoice_amount, user_amount, invoice_mid_value, user_mid_value):
    ...
    difference = total_user_amount - total_invoice_amount     #Difference in PLN
    
    if difference <= -0.01:
        print(f"An additional amount must be paid: {'{:.2f}'.format(abs(difference))} PLN")
    elif difference >= 0.01:
        print(f"You will receive a refund: {'{:.2f}'.format(difference)} PLN")
    else:
